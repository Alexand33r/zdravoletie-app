...
This project addresses the interpretability gap through a reverse-engineering approach. Rather than
...
ranking is statistically certifiable given the available data. Crucially, the design also allows a
...
plus joint angle features. This sequential structure ensures that any observed performance change
...
observed, and Sub-question~1 cannot be answered with a definitive model recommendation.
...
interval widens to [2.14, 4.31]. This behaviour is a direct consequence of the pseudo-label
distribution: the production model's predictions on the synthetic records have a standard deviation
...
This finding has a practical implication: synthetic augmentation through teacher-student
pseudo-labelling is not model-agnostic. Before applying it in an operational context, the
...
The RealPlusSynth versus SynthOnly comparison is not significant for any model. This is a nuanced
finding. It does not mean the real records are redundant. Under RealOnly, all five models achieve
...
The statistical power limitation identified in Section~\ref{sec:power} applies differently across
the three experiments and must be characterised with precision rather than applied as a blanket
disclaimer.

In Experiment~1, the power limitation is binding. All ten adjusted $p$-values are well above the
corrected threshold of 0.005, and the closest raw $p$-value (0.0043 for SVR versus
HistGradientBoosting) is already near the corrected threshold before adjustment. Increasing the
number of cross-validation splits from 30 to 50 would narrow the 95\% confidence intervals by
approximately 18\%, which could allow the SVR-versus-HistGradientBoosting comparison to reach
significance if the true effect size is close to what the point estimates suggest. That said, the
0.19-year mean MAE difference between these two models, compared to a fold-level standard
deviation of approximately 0.87 years, implies a modest effect size, and even a 50-split protocol
may not be sufficient.

In Experiment~2, the power limitation is not binding for the five significant comparisons, which
reach adjusted $p$-values as low as 0.000004. The effects detected there are large enough to be
identified even under the relatively conservative test setup. The limitation is relevant for the
non-significant regime comparisons, particularly the universally non-significant RealPlusSynth
versus SynthOnly comparisons, where small effects may simply be below the resolution of the
current test.

In Experiment~3, the power limitation is clearly binding. The lowest raw $p$-value observed is
0.21, for the G1-versus-G4 comparison --- a value that would not reach even the uncorrected 0.05
threshold. This suggests either that the true effect of adding bioimpedance, postural risk, and
joint angle features to the base group is genuinely negligible on the Anovator approximation task,
or that the effect exists but is smaller than the 30-fold signed-rank test can detect at this
sample size. The two hypotheses cannot be distinguished without either a larger dataset or a power
analysis conducted prior to data collection.

None of these observations are apologetic qualifications of a study that failed to find what it was
looking for. Underpowered tests that return non-significant results are honest results. The
obligation is to report them accurately and to not overstate what the available evidence supports.
...
the binding constraint on statistical power throughout all three experiments. Coordinating data
collection with other Anovator-equipped centres would reduce single-clinic bias, raise the power
of pairwise significance tests above the current inconclusive regime, and eventually support
time-based evaluation splits that are not feasible with the current longitudinal density.
